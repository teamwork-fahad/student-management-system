...
export const markAttendanceController = asyncHandler(async (req, res) => {
  ...
  await clearCachePatterns(["sms:attendance:*"]);
...
export const getAttendanceByDateController = asyncHandler(async (req, res) => {
  ...
  const cacheKey = buildCacheKey("sms:attendance:date", req.query);
  const students = await fetchWithCache(cacheKey, 60, () => getAttendanceByDate(date, courseId));
...
export const getAttendanceStatsController = asyncHandler(async (req, res) => {
  ...
  const cacheKey = buildCacheKey("sms:attendance:stats", req.query);
  const stats = await fetchWithCache(cacheKey, 60, () => getAttendanceStats(date));
...
export const getStudentAttendanceHistoryController = asyncHandler(async (req, res) => {
  ...
  const cacheKey = `sms:attendance:student:${studentId}`;
  const data = await fetchWithCache(cacheKey, 60, () => getStudentAttendanceHistory(studentId));
...
export const getPublicAttendanceReportController = asyncHandler(async (req, res) => {
  ...
  const cacheKey = buildCacheKey("sms:attendance:public", req.query);
  const report = await fetchWithCache(cacheKey, 60, () => getPublicAttendanceReport({ range }));
