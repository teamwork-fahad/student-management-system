...
export async function importInternshipStudents() {
  console.log("🚀 Starting Internship 2025 Student Import & Duplicate Check...");
  ...
  for (const item of RAW_SQL_DATA) {
    ...
    // Deduplication check against existing Student table
    ...
    if (existingStudent) {
      ...
      // Check if student already enrolled in Internship 2025 course
...
if (process.argv[1]?.includes('importInternshipStudents.js')) {
  importInternshipStudents()
    .then((stats) => {
      console.log("🎉 INTERNSHIP 2025 IMPORT COMPLETE!", stats);
    })
    .catch((err) => {
