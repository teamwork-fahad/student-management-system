...
import {
  ...
  DollarSign, Zap, Award, LayoutDashboard,
} from "lucide-react";
...
const getMonthlyData = (payments = []) => {
  const months = [];
  for (let i = 5; i >= 0; i--) {
    const d = new Date(); d.setDate(1); d.setMonth(d.getMonth() - i);
    const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
    months.push({ key, label: d.toLocaleString("default", { month: "short", year: "2-digit" }), amount: 0 });
  }
  payments.forEach((p) => {
...
const KPICardAnimated = ({ card, delay }) => {
  const animated = useCounter(card.target, 1400, delay + 200);
  return (
    <div className="kpi-card glass-card relative overflow-hidden rounded-2xl p-4 sm:p-5 border"
         style={{ background: card.cardBg, borderColor: card.border || "rgba(59,130,246,0.12)" }}>
      <div className="absolute -top-6 -right-6 w-24 h-24 rounded-full blur-3xl opacity-20 pointer-events-none orb-float"
           style={{ background: card.accent }} />
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-3">
          <p className="text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-tight pr-1">
            {card.label}
          </p>
          <div className="p-2 rounded-xl shrink-0" style={{ backgroundColor: card.iconBg, color: card.accent }}>
            {card.icon}
          </div>
        </div>
        <div className="text-xl sm:text-2xl xl:text-3xl font-black tracking-tight mb-1" style={{ color: card.accent }}>
          {card.displayFn(animated)}
        </div>
...
export const AnalyticsDashboard = () => {
  ...
  const fetchData = async () => {
    setLoading(true); setErrorMsg(""); setChartsReady(false);
    try {
      const [statsRes, admRes, feesRes, crsRes, expRes] = await Promise.all([
        api.get("/admissions/statistics"),
        api.get("/admissions?limit=15&sortBy=newest"),
        api.get("/fees?limit=300"),
        api.get("/courses"),
  ...
  const topCourses = Object.entries(cMap)
    ...
    .slice(0, 5);
  const maxCC = topCourses[0]?.count || 1;
  ...
  const kpiCards = [
    { label:"Total Students",      target:totalS,      displayFn:v=>v.toLocaleString("en-IN"), sub:`${activeC} currently active`,       icon:<Users className="w-4 h-4 sm:w-5 sm:h-5"/>,         accent:"#60a5fa", iconBg:"rgba(59,130,246,0.18)",  cardBg:"linear-gradient(145deg,#0d1b3e,#0f2255)" },
    { label:"Active Enrollments",  target:activeC,     displayFn:v=>v.toLocaleString("en-IN"), sub:"Currently studying",                 icon:<UserCheck className="w-4 h-4 sm:w-5 sm:h-5"/>,      accent:"#34d399", iconBg:"rgba(16,185,129,0.18)",  cardBg:"linear-gradient(145deg,#0d2a1e,#022c22)" },
    { label:"Fees Collected",      target:totalRev,    displayFn:v=>fmtK(v),                   sub:`${collRate}% collection efficiency`, icon:<CircleDollarSign className="w-4 h-4 sm:w-5 sm:h-5"/>,accent:"#4ade80", iconBg:"rgba(74,222,128,0.15)", cardBg:"linear-gradient(145deg,#0a2218,#032215)", trend:collRate>75?1:-1 },
    { label:"Pending Dues",        target:pendingAmt,  displayFn:v=>fmtK(v),                   sub:"Outstanding balance",                icon:<Clock className="w-4 h-4 sm:w-5 sm:h-5"/>,           accent:"#fb923c", iconBg:"rgba(251,146,60,0.18)", cardBg:"linear-gradient(145deg,#1c0e00,#1a0900)",  trend:pendingAmt>0?-1:1 },
    { label:"Total Courses",       target:courses.length, displayFn:v=>v.toLocaleString("en-IN"), sub:"Active programs",               icon:<BookOpen className="w-4 h-4 sm:w-5 sm:h-5"/>,          accent:"#a78bfa", iconBg:"rgba(167,139,250,0.18)",cardBg:"linear-gradient(145deg,#1a0d3e,#2d1b69)" },
    { label:"Graduated",           target:completedC,  displayFn:v=>v.toLocaleString("en-IN"), sub:"Successfully completed",            icon:<GraduationCap className="w-4 h-4 sm:w-5 sm:h-5"/>,    accent:"#38bdf8", iconBg:"rgba(56,189,248,0.15)", cardBg:"linear-gradient(145deg,#0d1b3e,#0c2a45)" },
    { label:"This Month",          target:curMon,      displayFn:v=>fmtK(v),                   sub:momChg!=null?(Number(momChg)>=0?`+${momChg}% vs last month`:`${momChg}% vs last month`):"Current month revenue", icon:<TrendingUp className="w-4 h-4 sm:w-5 sm:h-5"/>, accent:Number(momChg)>=0?"#4ade80":"#f87171", iconBg:Number(momChg)>=0?"rgba(74,222,128,0.15)":"rgba(248,113,113,0.15)", cardBg:"linear-gradient(145deg,#0f172a,#0d2a1e)", trend:momChg!=null?Number(momChg):null },
    { label:"Collection Rate",     target:collRate,    displayFn:v=>`${v}%`,                  sub:collRate>=80?"Excellent performance":collRate>=50?"On track":"Needs attention", icon:<Target className="w-4 h-4 sm:w-5 sm:h-5"/>, accent:collRate>=80?"#34d399":collRate>=50?"#fb923c":"#f87171", iconBg:"rgba(59,130,246,0.15)", cardBg:"linear-gradient(145deg,#0f172a,#0d1b3e)", trend:collRate>70?1:-1 },
  ];
    ...
    <>
      ...
      <div className="dash-root space-y-6 pb-16">
        ...
        <div className="s1 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center space-x-3 mb-1.5">
              ...
              <span className="hidden sm:flex items-center space-x-1.5 px-3 py-1 rounded-full border text-[10px] font-bold"
          ...
          <div className="flex items-center flex-wrap gap-2">
            ...
            <Link to="/dashboard/admissions"
              className="px-4 py-2 text-white text-xs font-bold rounded-xl flex items-center space-x-2 whitespace-nowrap"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 s2">
          ...
          <div className="glass-card lg:col-span-2 rounded-2xl border border-slate-800 p-5"
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
              ...
              <div className="text-right shrink-0">
                ...
                {momChg != null && (
                  <div className={`text-[10px] font-semibold flex items-center justify-end space-x-0.5 ${Number(momChg)>=0?"text-emerald-400":"text-rose-400"}`}>
                    {Number(momChg) >= 0 ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                    <span>{momChg}% MoM</span>
                  </div>
                )}
              </div>
            </div>
                ...
                return (
                  <div key={m.key} className="flex-1 flex flex-col items-center gap-1.5 group h-full">
                    <div className="flex-1 w-full flex items-end relative">
                      <div className="absolute -top-6 left-1/2 -translate-x-1/2 whitespace-nowrap text-[9px] font-bold text-white bg-slate-800 px-1.5 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" style={{zIndex:10}}>
                        {fmtK(m.amount)}
                      </div>
                      <div className="w-full rounded-t-lg bar-v relative overflow-hidden"
                           style={{
                             height: chartsReady ? `${Math.max(pct, 3)}%` : "3%",
                             background: isCur ? "linear-gradient(to top, #1d4ed8, #60a5fa)" : "linear-gradient(to top, #1e3a5f80, #2563eb50)",
                             boxShadow: isCur ? "0 0 16px rgba(59,130,246,0.4), 0 0 32px rgba(59,130,246,0.15)" : "none",
                    ...
                    <span className={`text-[9px] sm:text-[10px] font-semibold whitespace-nowrap ${isCur ? "text-blue-400" : "text-slate-500"}`}>
                      {m.label}
                    </span>
                  </div>
                );
            ...
            <div className="mt-5 pt-4 border-t border-slate-800/60 grid grid-cols-3 gap-4 text-center">
              {[
                ...
                <div key={item.label}>
                  ...
                  <div className="text-xs sm:text-sm font-extrabold" style={{ color: item.color }}>{item.val}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="glass-card rounded-2xl border border-slate-800 p-5 flex flex-col"
            ...
            <h3 className="text-sm font-bold text-white flex items-center space-x-2 mb-4">
              <Activity className="w-4 h-4 text-blue-400" /><span>Enrollment Status</span>
            </h3>
            <div className="flex-1 flex flex-col items-center justify-center">
              <div className="relative">
                <DonutChart segments={donut} size={136} thickness={18} ready={chartsReady} />
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-2xl font-black text-white leading-none">{totalS}</span>
                  ...
                  <div key={seg.label} className="flex items-center justify-between text-xs">
                    ...
                    <div className="flex items-center space-x-2 shrink-0">
                      <div className="w-16 h-1.5 rounded-full bg-slate-800 overflow-hidden">
                        <div className="h-full rounded-full bar-h"
                             style={{ width: chartsReady ? `${pct}%` : "0%", background: seg.color, transitionDelay: "0.4s" }} />
                      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 s3">
          ...
          <div className="glass-card rounded-2xl border border-slate-800 p-5"
            ...
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-sm font-bold text-white flex items-center space-x-2">
                <BookOpen className="w-4 h-4 text-blue-400" /><span>Top Enrolled Courses</span>
              </h3>
              <Link to="/dashboard/courses"
                    className="text-[11px] text-blue-400 hover:text-blue-300 flex items-center space-x-0.5 transition">
                  ...
                  return (
                    <div key={c.name} className="group">
                      <div className="flex items-center justify-between text-xs mb-1.5">
                        <div className="flex items-center space-x-2.5 min-w-0 flex-1">
                          <div className="w-6 h-6 rounded-lg text-[10px] font-black flex items-center justify-center shrink-0"
                               style={{ background: `${col}25`, color: col }}>{i + 1}</div>
                          <span className="font-semibold text-slate-200 truncate" title={c.name}>{c.name}</span>
                        </div>
                      ...
                      <div className="w-full h-2 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
                        <div className="h-full rounded-full bar-h"
                             style={{ width: chartsReady ? `${Math.max(pct, 5)}%` : "0%", background: `linear-gradient(90deg, ${col}80, ${col})`, boxShadow: `0 0 10px ${col}50`, transitionDelay: `${i * 0.1}s` }} />
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="glass-card rounded-2xl border border-slate-800 p-5"
            ...
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-sm font-bold text-white flex items-center space-x-2">
                <AlertCircle className="w-4 h-4 text-amber-400" /><span>High Pending Dues</span>
              </h3>
              {highPend.length > 0 && (
                    ...
                    <div key={adm.id}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-1 w-full h-1 rounded-full overflow-hidden bg-slate-800">
                          <div className="h-full rounded-full bar-h"
                               style={{ width: chartsReady ? `${dp}%` : "0%", background: urg, transitionDelay: `${i*0.08+0.5}s` }} />
                        </div>
                      </div>
                      <div className="text-right shrink-0">
                        <div className="text-xs font-extrabold" style={{ color: urg }}>{fmtK(pA)}</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 s4">
          ...
          <div className="glass-card rounded-2xl border border-slate-800 p-5 text-center"
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Collection Efficiency</p>
            <div className="flex justify-center relative mb-3">
              <svg width="120" height="70" viewBox="0 0 120 70">
                <path d="M 16 60 A 44 44 0 0 1 104 60" fill="none" stroke="#1e293b" strokeWidth="12" strokeLinecap="round"/>
                <path d="M 16 60 A 44 44 0 0 1 104 60" fill="none" stroke="url(#gaugeG)" strokeWidth="12" strokeLinecap="round"
                      strokeDasharray={`${chartsReady ? (collRate/100*138) : 0} 138`}
                      className="gauge-arc" style={{transitionDelay:"0.4s"}}/>
                <defs>
                  <linearGradient id="gaugeG" x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset="0%" stopColor="#1d4ed8"/><stop offset="100%" stopColor="#34d399"/>
                  </linearGradient>
                </defs>
              </svg>
          ...
          {/* Net Revenue */}
          <div className="glass-card rounded-2xl border border-emerald-900/30 p-5 text-center"
            ...
            <div className="border-t border-emerald-900/30 pt-3 grid grid-cols-2 gap-2 text-[10px]">
              <div><div className="text-slate-500">Revenue</div><div className="font-extrabold text-emerald-400">{fmtK(totalRev)}</div></div>
              <div><div className="text-slate-500">Expenses</div><div className="font-extrabold text-rose-400">{fmtK(totalExp)}</div></div>
            </div>
          </div>

          {/* Completion Rate */}
          <div className="glass-card rounded-2xl border border-blue-900/30 p-5 text-center"
               style={{ background: "linear-gradient(160deg, #0d1b3e 0%, #060d1f 100%)" }}>
            ...
            <p className="text-[10px] text-slate-500 mb-4">{completedC} of {totalS} students</p>
            <div className="w-full h-2.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
              <div className="h-full rounded-full bar-h"
                   style={{ width: chartsReady ? `${totalS>0?(completedC/totalS*100):0}%` : "0%", background:"linear-gradient(90deg,#1d4ed8,#3b82f6)", boxShadow:"0 0 12px rgba(59,130,246,0.4)", transitionDelay:"0.6s" }} />
            </div>
          </div>
        </div>

        {/* ── RECENT ADMISSIONS TABLE ────────────────────────────────────── */}
        <div className="glass-card rounded-2xl border border-slate-800 p-5 s4"
             style={{ background: "linear-gradient(160deg, #0d1b3e 0%, #0a0f1e 100%)" }}>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-0.5">Latest 15 enrollments &middot; Click any row to open student profile</p>
            </div>
                      ...
                      <tr key={adm.id} className="tr-hover cursor-pointer" onClick={() => window.open(url, "_blank")}>
                        ...
                        <td className="py-3 px-4 whitespace-nowrap">
                          <div className="flex items-center space-x-2.5">
                            ...
                            <div>
                              <div className="font-bold text-white text-xs leading-tight">{adm.student?.fullName || "N/A"}</div>
